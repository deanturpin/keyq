...
public class SimplePlayEngine {
    ...
    public init() {
        ...
        guard let fileURL = Bundle.main.url(forResource: "default_audio", withExtension: "wav") else {
            fatalError("\"default_audio.wav\" file not found.")
        }
    ...
    func setPlayerFile(_ fileURL: URL) {
        do {
            ...
            // Stop player and clear any scheduled buffers
            player.stop()
            ...
            if let avAudioUnit = avAudioUnit {
                ...
                engine.connect(player, to: avAudioUnit, format: file.processingFormat)
                engine.connect(avAudioUnit, to: engine.mainMixerNode, format: file.processingFormat)
            } else {
                engine.disconnectNodeInput(player)
